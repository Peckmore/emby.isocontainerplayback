...
{
    /// <summary>
    /// An <see cref="IsoStream"/> implementation which represents a video stored on a VideoCD ISO. The video may be spread across
    /// multiple files within the ISO.
    /// </summary>
    public class VideoCdIsoStream : IsoStream
    {
        ...
        public VideoCdIsoStream(string isoPath)
        {
            ...
            // We'll check whether a VideoCD DAT file exists on the ISO to know whether we have a valid ISO.
            var datPath = GenerateDatPath(1);
            if (!_cdReader.FileExists(datPath))
            {
                throw Exceptions.IsoVideoFileDoesNotExist(datPath);
            }
            ...
            for (var x = 1; x < 100; x++)
            {
                ...
                // Check whether the DAT exists, and add it to our list if it does.
                if (_cdReader.FileExists(datPath))
                {
                ...
                {
                    // DATs have to be continuous and sequential, so if we haven't found a DAT we've reached the end of the video
                    // files for this VideoCD.
                    break;
                }
            }
        }
        ...
        {
            // Generate the expected video file name for a DAT file, based on the part number.
